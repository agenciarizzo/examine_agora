...
import { clinica, href, landings, type Page, page, waHref } from '@/lib/content';
import { breadcrumb, graph } from '@/lib/jsonld';
import { metaDe } from '@/lib/meta';
import { cor, WRAP } from '@/lib/theme';

/** Landings vivem na raiz: /paaf-de-tireoide-brasilia, /ultrassom-abdominal-brasilia… */
const porSegmento = new Map(landings.map((p) => [p.path.replace(/^\//, ''), p]));

export const dynamicParams = false;

export function generateStaticParams() {
  return [...porSegmento.keys()].map((slug) => ({ slug }));
}
...
export async function generateMetadata({
  ...
  const p = porSegmento.get(slug);
  return p ? metaDe(p.slug) : {};
}
